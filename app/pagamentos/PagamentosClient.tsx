...
export default function PagamentosClient({
  ...
  async function loadPayments() {
    ...
    for (const attendance of attendances ?? []) {
      ...
      normalized.push({
        ...
        advance_value: existing
          ? Number(existing.advance_value)
          : 0,

        extra_value: existing
          ? Number(existing.extra_value)
          : 0,

        discount_value: existing
          ? Number(existing.discount_value)
          : 0,

        total_value: existing
          ? Number(existing.total_value)
          : dailyRate + transport,
  ...
  async function savePayment(
    ...
    const total =
      Number(updated.daily_rate || 0) +
      Number(updated.transport_value || 0) +
      Number(updated.extra_value || 0) -
      Number(updated.advance_value || 0) -
      ...
      .upsert(
        {
          ...
          transport_value: Number(
            updated.transport_value || 0
          ),

          advance_value: Number(
            updated.advance_value || 0
          ),

          extra_value: Number(
            updated.extra_value || 0
          ),
      ...
      <main className="dashboard-main">
        ...
        <header className="dashboard-header">
          ...
          <div>
            ...
            <p>
              Controle os valores devidos aos trabalhadores presentes.
            </p>
                  ...
                  {filteredRows.map((row) => {
                    ...
                    return (
                      <tr key={row.attendance_id}>
                        ...
                        <td>
                          {formatMoney(row.transport_value)}
                        </td>
                        ...
                        <td>
                          <input
                            ...
                            min="0"
                            step="0.01"
                            defaultValue={row.advance_value}
                            onBlur={(e) =>
                              savePayment(row, {
                                advance_value:
